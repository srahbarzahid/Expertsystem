...
function validateForm() {
  ...
  for (let i = 0; i < hyperparameters.length; i++) {
    ...
    if (hyperparameters[i].tagName.toLowerCase() === "select") {
      if (!value) {
        showWarningToast(
          `Please select a value for ${hyperparameters[i].name}.`,
        );
        return false;
      }
      ...
      showWarningToast(
        `Please enter a valid value for ${hyperparameters[i].name}.`,
      );
      return false;
    }
  }
...
function toggleHeatmaps() {
  document
    .querySelectorAll(".heatmaps")
    .forEach((heatmap) => heatmap.classList.toggle("d-none"));
}
  ...
  fetch("/get_scatter_plot/", {
    ...
    .then((data) => {
      try {
        ...
        window.scrollTo({
          top:
            document.getElementById("plotly-scatter").getBoundingClientRect()
              .top +
            window.scrollY -
            100,
          behavior: "smooth",
    ...
    introJs()
      .setOptions({
        steps: [
          {
            intro:
              "Great! Now you need to select the features and target variable. For the Iris dataset, we have 4 features and 1 target variable.",
          },
          {
            element: document.querySelector("#features-div"),
            intro:
              "Select the features of the Iris flower you want to use to predict the species. Let's predict the species based on the sepal length (SepalLengthCm) and sepal width (SepalWidthCm).",
          },
          {
            element: document.querySelector("#target-div"),
            intro:
              "For the Iris dataset, it's the 'Species' column that you want to predict. Select 'Species' as the target variable.",
          },
          {
            element: document.querySelector("#n_neighbors"),
            intro:
              "A hyperparameter is a parameter whose value is set before the learning process begins. KNN has a hyperparameter called 'n_neighbors', which is the number of neighbors to consider when classifying a data point. Let's set it as 5.",
          },
          {
            element: document.querySelector("#canvas-1"),
            intro:
              "This is a heatmap of the Iris dataset. It shows the correlation between each pair of features. The darker the color, the higher the correlation.",
          },
          {
            element: document.querySelector("#canvas-2"),
            intro:
              "This is a scatter plot of the Iris dataset. It shows the relationship between a pair of features. You can select the features you want to plot on the X and Y axes to visualize the data.",
          },
          {
      ...
      .start();
    (document.querySelector("#build-btn"),
