...
def lasso(request):
    ...
    if request.method == 'POST':
        ...
        features, target, test_size, alpha, max_iter, tol = get_input(
            request.POST, 'alpha', ('max_iter', 1000), ('tol', 1e-4))
...
def decision_tree_regression(request):
    ...
    if request.method == 'POST':
        ...
        features, target, test_size, max_depth, min_samples_split = get_input(
            request.POST, ('max_depth', None), ('min_samples_split', 2))
        ...
        if not max_depth:
            ...
            model = DecisionTreeRegressor(max_depth=int(
                max_depth), min_samples_split=min_samples_split, random_state=42)
        model.fit(X_train, y_train)
...
def random_forest_regression(request):
    ...
    if request.method == 'POST':
        ...
        features, target, test_size, n_estimators, max_depth, min_samples_split = get_input(
            request.POST, 'n_estimators', ('max_depth', None), ('min_samples_split', 2))
        ...
        if not max_depth:
            ...
            model = RandomForestRegressor(n_estimators=n_estimators, max_depth=int(
                max_depth), min_samples_split=min_samples_split, random_state=42)
        model.fit(X_train, y_train)
...
def knn(request):
    ...
    if request.method == "POST":
        ...
        features, target, test_size, n_neighbors, weights, algorithm, metric, p = get_input(
            request.POST, 'n_neighbors', 'weights', 'algorithm', 'metric', 'p')
    ...
    return render(request, 'main/input.html', {
        ...
        'optional_parameters': [
            {'field': 'select', 'name': 'weights', 'type': 'text',
                'options': ['uniform', 'distance'], 'default': 'uniform'},
            {'field': 'select', 'name': 'algorithm', 'type': 'text', 'options': [
                'auto', 'ball_tree', 'kd_tree', 'brute'], 'default': 'auto'},
            {'field': 'select', 'name': 'metric', 'type': 'text', 'options': [
                'minkowski', 'euclidean', 'manhattan', 'cosine', 'hamming'], 'default': 'minkowski'},
...
def logistic_regression(request):
    ...
    return render(request, 'main/input.html', {
        ...
        'datasets': list_available_datasets('classification'),
        'optional_parameters': [
            {'field': 'select', 'name': 'solver', 'type': 'text', 'options': [
                'newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga'], 'default': 'lbfgs'},
            {'field': 'select', 'name': 'penalty', 'type': 'text', 'options': [
                'l2', 'None', 'elasticnet', 'l1'], 'default': 'l2'},
...
def random_forest(request):
    ...
    if request.method == "POST":
        ...
        if not max_depth:
            ...
            model = RandomForestClassifier(n_estimators=n_estimators, max_depth=int(
                max_depth), min_samples_split=min_samples_split, random_state=42)
        model.fit(X_train, y_train)
...
def svm(request):
    ...
    if request.method == "POST":
        ...
        features, target, test_size, kernel, C, gamma, degree = get_input(
            request.POST, 'kernel', 'C', 'gamma', 'degree')
...
def hierarchical_clustering(request):
    ...
    return render(request, 'main/input_clustering.html', {
        ...
        'optional_parameters': [
            ...
            {'field': 'select', 'name': 'linkage_method', 'type': 'text', 'options': [
